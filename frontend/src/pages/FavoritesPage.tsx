    ...
    const handleToggleFavorite = async (e: React.MouseEvent, code: string) => {
        ...
        try {
            ...
            if (selectedStock && selectedStock.stock.code === code) {
                setSelectedStock({
                    ...selectedStock,
                    stock: { ...selectedStock.stock, is_favorite: isFav }
                });
                if (!isFav) {
                    // Should we close modal? Maybe keep it open.
                }
            }
        } catch (err) {
            console.error(err);
    ...
    return (
        <div className="max-w-6xl mx-auto">
            <header className="mb-10">
                ...
                <p className="text-gray-500 mt-2">Create your own portfolio watchlist</p>
            </header>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                ...
                {stocks.length === 0 && (
                    <div className="col-span-full text-center py-20 text-gray-400">
                        No favorite stocks yet. Add some from the home page!
                    </div>
                )}
            </div>

            {/* Detail Modal (Duplicated code - should be a component ideally) */}
            <AnimatePresence>
                {selectedStock && (
                    <motion.div
                        initial={{ opacity: 0 }}
